...
public:
    ...
    GateLFile &operator=(const GateLFile &) = delete; // assignment operator
    GateLFile(const GateLFile &GateLFile) = delete;   // copy constructor

    int Get(const char *fileName, const std::string &topModule);
    int GetLib(const char *fileName);
